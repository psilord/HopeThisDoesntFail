...
(defun make-spritesheet-buffer (spritesheet)
  (with-slots (%spec %buffer) spritesheet
    ;; TODO: This 2 is hardcoded because virality doesn't have an allocation
    ;; system for these integers.
    (gpu:bind-block :htdf/ship-spritesheet 2)
    (setf %buffer (gpu:create-buffer (a:make-gensym :htdf/ship-spritesheet)
                                     :htdf/ship-spritesheet))
    ;; TODO: Have materials automatically calculate a binding point instead of
    ;; hard-coding.
    (gpu:bind-buffer (gpu:buffer-name %buffer) 2)
...
(defun draw-sprite (sprite &optional count)
  (with-slots (%index %spritesheet) sprite
    (with-slots (%geometry) %spritesheet
      (gpu:uniform-int 'htdf/shd:sprite :sprite.index %index)
